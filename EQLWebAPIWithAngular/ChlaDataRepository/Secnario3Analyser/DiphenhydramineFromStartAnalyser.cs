...
{
    class DiphenhydramineFromStartAnalyser : Analyser
    {
        ...
        protected override JObject AnalyseAction(JObject jsonObject)
        {
            ...
            if (jarr != null)
            {
                ...
                foreach (var jobj in jarr)
                {
                    ...
                    else if (currentrow.GetValue("ActionID")?.ToString() == "MEDICATION_USED" && currentrow.GetValue("ActionValue")?.ToString() == "BenadrylIVMedication" && currentrow.GetValue("ActionOutcome")?.ToString() == "ACTIVATED")
                    {
                        med = currentrow.GetValue("Event_Time")?.ToString();
                    }
                    else if (currentrow.GetValue("ActionID")?.ToString() == "MEDICATION_USED" && currentrow.GetValue("ActionValue")?.ToString() == "BenadrylTabletMedication" && currentrow.GetValue("ActionOutcome")?.ToString() == "ACTIVATED")
                    {
                        med = currentrow.GetValue("Event_Time")?.ToString();
                    }
                }
