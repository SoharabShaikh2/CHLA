...
{
    class MethylprednisoloneFromStartAnalyser : Analyser
    {
        ...
        protected override JObject AnalyseAction(JObject jsonObject)
        {
            ...
            if (jarr != null)
            {
                ...
                foreach (var jobj in jarr)
                {
                    ...
                    else if (currentrow.GetValue("ActionID")?.ToString() == "MEDICATION_USED" && currentrow.GetValue("ActionValue")?.ToString() == "SoluMedrolIVMedication" && currentrow.GetValue("ActionOutcome")?.ToString() == "ACTIVATED")
                    {
                        med = currentrow.GetValue("Event_Time")?.ToString();
                    }
                    else if (currentrow.GetValue("ActionID")?.ToString() == "MEDICATION_USED" && currentrow.GetValue("ActionValue")?.ToString() == "PrednisoneTabletMedication" && currentrow.GetValue("ActionOutcome")?.ToString() == "ACTIVATED")
                    {
                        med = currentrow.GetValue("Event_Time")?.ToString();
                    }
                }
